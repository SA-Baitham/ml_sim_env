...
class PickCubeEnv(MujocoEnv):
    ...
    def collect_data_sequence(self):
        ...
        while not terminated:
            ...
            if env_state == EnvState.APPROACH:
                ...
                target_pos.position[2] = self.first_target_pose.position[2] + 0.3
                grip_angle = 250
            else:
                target_pos.position[0] = self.first_target_pose.position[0] + 0.25
                target_pos.position[1] = self.first_target_pose.position[1] + 0.1
                target_pos.position[2] = self.first_target_pose.position[2] + 0.4
                grip_angle = 250

            # target orientation 생성

            if env_state == EnvState.APPROACH or env_state == EnvState.PICK:
                euler_ = euler.quat2euler(cur_qpos[-4:])
                ...
                target_pos.orientation[:] = cur_qpos[-4:]
                grip_angle = 250
            ...
            if env_state == 4:
                waypoints = self.make_trajectory_for_4(target_pos, 50)
    ...
    def make_trajectory_by_pose(self, target_pose, length):
        ...
        trajectory = interpolate_trajectory(start, end, 50)

        # print(" sample trajectory : ", trajectory)
        return trajectory

    def make_trajectory_for_4(self, target_pose, length):
        ...
        end = np.append(end_pose, start_quat)
